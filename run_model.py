...
# Copyright 2020 DeepMind Technologies Limited. A     ll Rights Reserved.
#  
...
FLAGS = flags.FLAGS
flags.DEFINE_enum('model', 'cloth', ['cloth', 'deform'], #'deform'
                  'Select model to run.')
flags.DEFINE_enum('mode', 'all', ['train', 'eval', 'all'], ## all
    ...
    for epoch in range(run_step_config['epochs'])[trained_epoch:]:
        ...
        if len(epoch_run_times) != 0:
            ...
            if hpc_current_time + epoch_mean_time >= hpc_max_time:
                ...
                is_train_break = True
                break
        print(run_step_config['dataset_dir'])
        #sys.exit()
...
def main(argv):
    ...
    dataset_dir = os.path.join('data_creation', dataset_name)  ## modified

    #dataset_dir = os.path.join('Data', dataset_name)  ## original 
